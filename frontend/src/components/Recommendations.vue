<template>
	...
	<div>
		...
		<div class="recommendations-container">
			...
			<div
				v-for="book in recommendations.slice(0, recommendations.length / 2)"
				class="book"
				:key="book.id"
			>
		...
		<div class="recommendations-container">
			...
			<div
				v-for="book in recommendations.slice(recommendations.length / 2, recommendations.length)"
				class="book"
				:key="book.id"
			>
				<router-link :to="`/book-info/${book.id}`">
		...
		fetch( // Get recommended books
			this.$backend_url + '/users/recommendations',
			{
				...
				data => {
					...
					fetch(
						this.$backend_url + `/books/search?query=subject:${mostViewed}&limit=16`,
						{
							headers: {
								Authorization: token
							}
