...
<router-link :to="'/user/' + user.id" class="container-link" v-if="currentUser !== null">
    <table style="width: 100%;"><tr style="width: 100%;">
        <td style="width: 100px;">
            <img
                :src="user.avatar_url ? user.avatar_url : require('../assets/BookSploreIcon.svg')"
                height="80px"
                width="80px"
				class="profile"
            />
            ...
            <tr style="width: 100%">
                <div style="display: flex; justify-content: column; width: 100%; padding-bottom: 10px;">
                    <h3>
                        {{user.username}}
                        <div class="follower">
                            {{ JSON.parse(user.followers).length }} Followers | {{ JSON.parse(user.following).length }} Following
                        </div>
                    </h3>
                </div>
				<td>
					<div class="button-container">
						<div class="follow" @click="follow()" :class="[ user.id == currentUser.id ? 'same' : 'different' ]">
