    ...
    async with aiorequest('GET', url, params=params) as response:
        ...
        for book in all_data:
            ...
            await request.app.state.db.execute(
                "INSERT INTO cached_searches (book_id, response) VALUES ($1, $2) ON CONFLICT DO NOTHING", id,
...
async def get_books_data(request: Request, book_id: str, authorization: Optional[str] = Header(None)):
    ...
    if book_data:
        ...
        await db.execute(cache_query, book_info['id'], book_info.get('title'), json.dumps(book_info.get('image_links')),
